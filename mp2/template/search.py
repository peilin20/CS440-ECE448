...
def bfs(maze, ispart1=False):
    ...
    """
    que=deque()
    que.append(maze.getStart())
    ##initialize queue for maze
    explored=set()
    explored.add(maze.getStart())
    prev_dict={}
    ret_path=deque()
    #initialize explored set of points
    #initialize dictionary for previous points
    ...
    while que :
        ...
        if maze.isObjective(curr[0],curr[1],curr[2],ispart1):
            ret_path.appendleft(curr)
            while ret_path[0] !=maze.getStart():
                ret_path.appendleft(prev_dict[curr])
                curr=prev_dict[curr]
            return ret_path
        
        for points in maze.getNeighbors(curr[0],curr[1],curr[2],ispart1):
            if points not in explored:
                prev_dict[points] = curr
                que.append(points)
                explored.add(points)

...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    """
    ##initialize bounds
    height=int (window[1] / granularity + 1)
    wid=int (window[0] / granularity + 1)
    offset=[0,0,0]
    ##initialize all tge possible ASCII
    SPACE = ' '
    WALLS = '%'
    GOALS = '.'
    START = 'P'
    shape=('Horizontal','Ball','Vertical')
    input_map = np.full((wid,height,3),' ')
    for k in range(3):
        for j in range(height):
            for i in range(wid):
                ##for each point in map
                config = idxToConfig((i, j, k), offset, granularity, alien)
                alien_obj = copy.deepcopy(alien)
                alien_obj.set_alien_pos((config[0], config[1]))
                alien_obj.set_alien_shape(shape[k])
                if (is_alien_within_window(alien_obj, window, granularity)==False):
                    input_map[i][j][k] = WALLS
                elif (does_alien_touch_wall(alien_obj, walls, granularity)==True):
                    input_map[i][j][k] = WALLS
                elif (does_alien_touch_goal(alien_obj, goals)==True):
                    input_map[i][j][k] = GOALS
    index = configToIdx(alien.get_config(), offset, granularity, alien)
    input_map[index[0]][index[1]][index[2]] = START

    ret = Maze(input_map, alien, granularity)
    return ret
